...
Deno.serve(async (req: Request) => {
  ...
  const workerToken = req.headers.get("x-dc-worker-token");
  const validSecrets = Object.values(secretKeys).filter(Boolean);

  let authorized = Boolean(providedKey && validSecrets.includes(providedKey));

  if (!authorized && workerToken) {
    const { data: workerAuthorized, error: workerAuthError } = await admin.rpc(
      "validate_worker_token",
      {
        p_token_key: "notification-delivery-worker",
        p_token: workerToken,
      },
    );
