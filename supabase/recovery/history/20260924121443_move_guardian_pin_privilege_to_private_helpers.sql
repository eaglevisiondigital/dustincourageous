...
create or replace function private.set_guardian_pin_impl(
  p_household_id uuid,
  p_pin text
)
returns void
...
create or replace function private.guardian_pin_status_impl(
  p_household_id uuid
)
returns table (
  configured boolean,
...
create or replace function private.verify_guardian_pin_impl(
  p_household_id uuid,
  p_pin text
)
returns boolean
...
revoke all on function private.set_guardian_pin_impl(uuid,text) from public, anon;
revoke all on function private.guardian_pin_status_impl(uuid) from public, anon;
revoke all on function private.verify_guardian_pin_impl(uuid,text) from public, anon;
grant execute on function private.set_guardian_pin_impl(uuid,text) to authenticated;
grant execute on function private.guardian_pin_status_impl(uuid) to authenticated;
grant execute on function private.verify_guardian_pin_impl(uuid,text) to authenticated;
...
set search_path = ''
as $$
  select private.set_guardian_pin_impl(p_household_id, p_pin);
$$;
...
set search_path = ''
as $$
  select * from private.guardian_pin_status_impl(p_household_id);
$$;
...
set search_path = ''
as $$
  select private.verify_guardian_pin_impl(p_household_id, p_pin);
$$;
