...
export async function readNotificationPage(client: SupabaseClient<Database>, userId: string, cursor: NotificationCursor | null = null, filter: NotificationFilter = "all") {
  if (!uuid.test(userId) || (cursor && !validCursor(cursor)) || !["all", "unread"].includes(filter)) throw new Error("Notifications could not be requested.");
  ...
  if (!data || data.some(row => row.user_id !== userId || !validCursor(row) || (filter === "unread" && row.status !== "unread") ||
