...
export function FamilyAssignments({ householdId, children }: { householdId: string; children: FamilyChild[] }) {
  ...
  return <section className="family-section-card">
    ...
    {loading && <p role="status">Loading family assignments...</p>}
    {error && <p role="alert">Family assignments could not be loaded. Use Refresh Assignments to try again.</p>}
    {!loading && !error && !items.length && <p className="muted">No assignments are available for your active child profiles yet. Family challenges and Faith At Home are available below when published.</p>}
    {!!items.length && <>
      <label>Choose Assignment<select value={activeId} disabled={working || loading || error} onChange={event => setActiveId(event.target.value)}>
      ...
      {active?.challenge && <>
        ...
        <FamilyChallengeActivity key={active.id} householdId={householdId} children={children} challengeId={active.challenge.id} onBusyChange={setWorking} suspended={loading || error}/>
      </>}
    </>}
  </section>;
}
